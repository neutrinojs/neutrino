...
Use a chaining API to generate and simplify the modification of webpack configurations.

This documentation corresponds to v4 of webpack-chain, which Neutrino utilizes.

_Note: while webpack-chain is utilized extensively in Neutrino, the package is completely standalone and can be used by any project. See the webpack-chain repo for standalone documentation._
...
In the Neutrino API, the `config` property is an instance of webpack-chain. For this guide, our example base configuration will be `.neutrinorc.js` in the root of our project directory. This can utilize either the [Object format](middleware.md#object-format) or [Function format](middleware.md#function-format) to access the Neutrino API:
...
For details on the specific values that are valid for all shorthand and low-level methods, please refer to their corresponding name in the [webpack docs hierarchy](https://webpack.js.org/configuration/).
...
_NOTE: Do not use _`new`_ to create the plugin, as this will be done for you._
